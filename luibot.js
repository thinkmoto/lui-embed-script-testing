window.addEventListener('load', function () {
  ...
  if (typeof LEX_LUI !== 'undefined') {
    if (typeof LEX_LUI.url !== 'undefined') {
      // path to lui folder on the server
      let constructPathToLui = (url) => {
        let pathToLui = url.split('/');
        let rightIndex = pathToLui.indexOf('lui');
        let newPath = [];
        for (let i = 0; i < rightIndex; i++) {
          newPath.push(pathToLui[i]);
        }
        return newPath.join('');
      };
      const pathToLui = constructPathToLui(LEX_LUI.url);
      console.log(pathToLui);
      const style = document.createTextNode(`
        ...
        @font-face {
          ...
          src: url('${pathToLui}/lui/assets/fonts/lexoffice/hlf-lexoffice.woff');
        }
        .lui-embedded__close {
          cursor: pointer;
          font-size: 30px;
      ...
      @font-face{
          ...
          src: url('${pathToLui}/lui/assets/fonts/open-sans-v17-latin/open-sans-v17-latin-regular.woff2');
          font-weight: 400;
      }
      ...
      let appear = lottie.loadAnimation({
        ...
        loop: false,
        name: 'appear',
        path: `${pathToLui}/lui/assets/botanimations/lui-appears.json`,
        renderer: 'svg',
      });
      ...
      }#cid=${cid ? cid : fallbackCid}&pid=${pid}&pdata=${pdata}`;
